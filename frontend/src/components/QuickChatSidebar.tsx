...
import { Check, Home, MessageSquare, PanelLeftClose, PanelLeftOpen, Pencil, Plus, Trash2, X } from "lucide-react";
...
const COLLAPSE_STORAGE_KEY = "adm_quickchat_sidebar_collapsed";
...
// Quick Chat's own local sidebar (Phase 4) — the standalone page's only
// chrome besides the chat itself. No Projects/Skills/Upload nav (that's
// the dashboard's job); just chat history + a way back home, same rename/
// delete affordances the old global sidebar had for this same chat list.
// Collapsible to a narrow icon rail (persisted per-browser) so the chat
// area can reclaim the width once the list isn't needed.
export const QuickChatSidebar: React.FC<Props> = ({ onHome }) => {
  ...
  return (
    <aside className="relative z-10 w-64 h-full flex flex-col border-r border-slate-200 bg-white/80 backdrop-blur-xl shrink-0">
      <div className="p-3 border-b border-slate-200 space-y-2">
        <div className="flex items-center gap-1">
          <button
            type="button"
            onClick={onHome}
            className="flex-1 flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-900 transition-colors cursor-pointer shrink-0"
          >
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        ...
        {chats.length === 0 && <p className="px-2.5 py-2 text-[11px] text-slate-400">No chats yet — send a message to start one.</p>}
      </div>
