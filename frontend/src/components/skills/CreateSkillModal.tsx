...
export const CreateSkillModal: React.FC<Props> = ({ projectId, onClose, onCreated }) => {
  ...
  const pollDraft = (id: string) => {
    setDraftId(id);
    // Also bounds how long "not written yet" is trusted — a draft_id that
    // never appears after 60s (~40 polls) is treated as failed rather than
    // polled forever, in case a future failure path forgets to write the
    // status="failed" placeholder normalize_skill_task_async now writes.
    let attempts = 0;
    pollRef.current = window.setInterval(async () => {
      ...
      try {
        ...
        if (draft.status === "failed") {
          if (pollRef.current) window.clearInterval(pollRef.current);
          setError(draft.error || "Failed to process this skill.");
          setPhase("compose");
          return;
        }
        if (pollRef.current) window.clearInterval(pollRef.current);
        ...
        if (err instanceof ApiError && err.status === 404 && attempts < 40) return; // draft not written yet — keep polling
        if (pollRef.current) window.clearInterval(pollRef.current);
      ...
      const merged: SkillDraftExtracted = {
        ...
        task_list:
          (form.kind || "task") === "workflow"
            ? (form.task_list || []).filter((t) => t.task_id.trim() && t.skill_id.trim())
            : [],
      };
  ...
  // Generated by the Skill Normalizer from a multi-step source description
  // (only present/relevant for kind="workflow") — editable here rather
  // than requiring the user to author task_list by hand.
  const updateTaskRow = (index: number, field: keyof SkillWorkflowTask, value: string) => {
    setForm((prev) => {
  ...
  const addTaskRow = () => {
    setForm((prev) => ({ ...prev, task_list: [...(prev.task_list || []), { task_id: "", skill_id: "", name: "" }] }));
  };

  const removeTaskRow = (index: number) => {
    setForm((prev) => ({ ...prev, task_list: (prev.task_list || []).filter((_, i) => i !== index) }));
  };
                ...
                <div className="space-y-1.5 bg-slate-50 border border-slate-200 rounded-xl p-3">
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] font-bold text-slate-700">
                      Generated Tasks — the commands this workflow runs, in order
                    </label>
                    <button
                      type="button"
                      onClick={addTaskRow}
                      title="Add task"
                      className="p-1 rounded-lg text-brand-orange hover:bg-brand-orange-light cursor-pointer"
                    >
                  ...
                  {(form.task_list || []).length === 0 ? (
                    <p className="text-[11px] text-slate-400 italic">
                      No steps drafted — add one, or rephrase the source description to spell out the steps.
                    </p>
                  ) : (
                      ...
                      {(form.task_list || []).map((t, i) => (
                        <div key={i} className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg p-1.5">
                          <div className="flex-1 space-y-1">
                            <input
                              value={t.name || ""}
                              onChange={(e) => updateTaskRow(i, "name", e.target.value)}
                              placeholder="Display name (e.g. Profile Source)"
                              className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px] font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-orange/30"
                            />
                            <div className="flex items-center gap-1.5">
                              <input
                                value={t.task_id}
                                onChange={(e) => updateTaskRow(i, "task_id", e.target.value)}
                                placeholder="task_id"
                                className="w-1/3 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px] font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-orange/30"
                              />
                              <input
                                value={t.skill_id}
                                onChange={(e) => updateTaskRow(i, "skill_id", e.target.value)}
                                placeholder="skill_id to run (e.g. profile_source)"
                                className="flex-1 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[11px] font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-orange/30"
                              />
                            </div>
                          </div>
                          <button
