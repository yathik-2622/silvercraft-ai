...
async def _test_connection(dialect: str, host: str, port: Optional[int], user: str, password: str, database: str, schema: str) -> Dict[str, Any]:
    """
    Attempt a live DB connection test and parse full AST/schema.
    ...
    try:
        ...
        if dialect == "postgres":
            url = f"postgresql+asyncpg://{urllib.parse.quote_plus(user)}:{urllib.parse.quote_plus(password)}@{host}:{port or 5432}/{database}"
        elif dialect in ("mysql", "mariadb"):
            url = f"mysql+aiomysql://{urllib.parse.quote_plus(user)}:{urllib.parse.quote_plus(password)}@{host}:{port or 3306}/{database}"
        else:
            return {"ok": True, "tables": [], "error": None, "note": f"Dialect '{dialect}' schema parsing not implemented — saved without live test."}

        engine = create_async_engine(url, echo=False)
        
        def _inspect_schema(conn):
            inspector = inspect(conn)
            table_names = inspector.get_table_names(schema=schema if schema else None)
            parsed_tables = []
            
            for table_name in table_names[:200]: # Limit for safety
                cols = inspector.get_columns(table_name, schema=schema if schema else None)
                pks = inspector.get_pk_constraint(table_name, schema=schema if schema else None)
                fks = inspector.get_foreign_keys(table_name, schema=schema if schema else None)
        ...
        async with engine.connect() as conn:
            parsed_tables = await conn.run_sync(_inspect_schema)
            
        await engine.dispose()
        return {"ok": True, "tables": parsed_tables, "error": None}

    except ImportError as exc:
        return {"ok": False, "tables": [], "error": f"Driver not installed: {exc}. Install the appropriate async driver (e.g. asyncpg, aiomysql, sqlalchemy)."}
